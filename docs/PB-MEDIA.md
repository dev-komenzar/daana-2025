...
### FigureExtension と renderHTML

TipTap エディタ内で `<figure>` 要素を扱う `FigureExtension` (`src/lib/cms/editor/figure-extension.ts`) は、画像の `src` をノード属性として保持するが、`renderHTML` の実装に注意が必要。

**発生した問題**: 属性 spec の `renderHTML: () => ({})` が `src`/`alt`/`caption` を `HTMLAttributes` から除外していたため、ノードの `renderHTML({ HTMLAttributes })` で `src ?? ''` が常に空文字になり、編集画面で画像が表示されなかった。

**正しい実装**: ノードの `renderHTML` では `HTMLAttributes` ではなく `node.attrs` から直接属性値を読む。

```ts
// 誤: HTMLAttributes 経由 (属性 spec の renderHTML が {} を返すと値が欠落)
renderHTML({ HTMLAttributes }) {
  const { src } = HTMLAttributes   // ← 常に undefined
  return ['figure', {}, ['img', { src: src ?? '' }]]  // ← src=""
}

// 正: node.attrs から直接読む
renderHTML({ node }) {
  const attrs = node.attrs as { src?: string }
  return ['figure', {}, ['img', { src: attrs.src ?? '' }]]
}
```
