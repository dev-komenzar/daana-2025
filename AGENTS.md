...
思考言語は指定なし。ユーザーには日本語で話して。commit, subagents などのプログラミング用語は英語のままでよいです。
...
4. **beads の永続化** - **必須**：`bd dolt push` で refs/dolt/data を更新（JSONL は使わない）
5. **コード変更をcommit** - **必須**：
   ```bash
   git add <files>
   git commit -m "..."
   ...
   ```
6. **クリーンアップ** - stashの解消、不要なファイルの削除
7. **検証** - すべての変更がcommit済み・beads が push 済みであること
8. **引き継ぎ** - 次のセッションへのコンテキストを提供する
...
- **dev server を起動しない**: `pnpm dev` 等の dev server はエージェントから起動しないこと。複数インスタンスでポートが重複するため、dev server はユーザーが手動で起動する。検証はユーザーが起動済みの dev server に Playwright MCP 等を通してアクセスして行う。
...
**Important**: プレビュー・開発サーバーの起動はユーザーが行います。エージェントは `pnpm dev` や `pnpm preview` を実行しないでください。
